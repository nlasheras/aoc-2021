...
class BITStream:
    def __init__(self, input):
        ...
        self.__bytes__ = [int(c, 16) for c in input]
        self.__pos__ = 0
        self.__len__ = 4 * len(input)
...
def parse_packet(stream):
    start_pos = stream.pos()
    p = Packet()
    p.version = stream.read(3)
    p.type_id = stream.read(3)
    if p.type_id == 4:
        p.literal, _ = parse_literal(stream)
    else:
        ...
        if length_id == 0:
            ...
            while to_read_count:
                ...
                to_read_count -= 1
                p.packets += [packets]
    return p, stream.pos() - start_pos

def part1_sum_version(input, expected_value = None):
    stream = BITStream(input)
    p, _ = parse_packet(stream)
...
def part2_evaluate(input, expected_value = None):
    stream = BITStream(input)
